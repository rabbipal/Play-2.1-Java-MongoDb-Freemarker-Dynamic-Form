...
public class Freemarker {
	...
	public static Html view(String template, Arg... args) {
		...
		try {
			...
			root.put(
					"Router",
					new BeansWrapper().getStaticModels().get(
							"controllers.routes"));
			...
			cfg.getTemplate(firstTemplate).process(root, out);

			out.flush();

			return Html.apply(out.toString());
		} catch (FileNotFoundException e) {
			throw new TemplateNotFoundException(template, Thread
					.currentThread().getStackTrace());
		} catch (freemarker.core.ParseException e) {
			throw new ExceptionInTemplate(template, e.getLineNumber(),
					e.getColumnNumber(), e.getMessage(), e);
		} catch (IOException ex) {
			if (ex.getCause() instanceof freemarker.core.ParseException) {
				freemarker.core.ParseException e = (freemarker.core.ParseException) ex
						.getCause();
				throw new ExceptionInTemplate(template, e.getLineNumber(),
						e.getColumnNumber(), e.getMessage(), e);
			}
			...
			Integer position = Integer
					.parseInt(grep("column ([0-9]+)", ftStack));
			String tmpl = grep("in ([^ \\]]+)", ftStack);
			throw new ExceptionInTemplate(tmpl, line, position,
					ex.getMessage(), ex);
		} finally {
	...
	public static class ExceptionInTemplate extends
			play.api.PlayException.ExceptionSource {
		final String template;
		final Integer line;
		final Integer position;

		public ExceptionInTemplate(String template, Integer line,
				Integer position, String description, Throwable cause) {
		...
		public String input() {
			InputStream is = Play.application().resourceAsStream(
					"/views/" + template);
